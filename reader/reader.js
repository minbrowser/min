...
autoReaderCheckbox.checked =
  readerDecision.getDomainStatus(articleURL) === true;
...
function extractAndShowNavigation(doc) {
  ...
  try {
    ...
    const currentDir = articleLocation.pathname
      .split("/")
      .slice(0, -1)
      .join("/");
    ...
    var items = Array.from(
      ...
      .filter((el) => {
        ...
        if (
          ...
          articleLocation.hostname.replace("www.", "")
        ) {
          return false;
        }
    ...
    items.forEach(function (item) {
      ...
      if (
        realURL.pathname !== "/" &&
        articleURL.startsWith(realURL.toString())
      ) {
...
function extractDate(doc) {
  var date;
  var dateItem = doc.querySelector(
...
function setReaderFrameSize() {
  ...
  rframe.height =
    rframe.contentDocument.body.querySelector(".reader-main").scrollHeight *
      1.01 +
    "px";
}
...
function startReaderView(article, date) {
  ...
  if (!article) {
    // we couln't parse an article
    readerContent +=
      "<div class='reader-main'><em>No article found.</em></div>";
  } else {
    if (article.title) {
