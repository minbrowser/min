...
var url = new URLSearchParams(window.location.search.replace("?", "")).get(
  "url"
);
...
function updateGutterWidths() {
  ...
  if (!pageViews[0]) {
    ...
    gutterWidth =
      Math.round(
        Math.max(64, (window.innerWidth - pageViews[0].viewport.width) / 2)
      ) - 2;
  }
...
function setupPageDom(pageView) {
  ...
  if (
    ...
    !pageView.annotationLayer.div.parentNode
  ) {
    ...
    for (var i = 1; i <= pageCount; i++) {
      ...
      await pdf.getPage(pageNumber).then(function (page) {
        ...
        (function (pageNumber, pdfPageView) {
          setTimeout(function () {
            if (
              pageNumber < pageBuffer ||
              (currentPage && Math.abs(currentPage - pageNumber) < pageBuffer)
            ) {
              pageContainer.classList.add("loading");
              pdfPageView
                .draw()
                .then(function () {
                  setupPageNavigation(pdfPageView);
                })
                .then(function () {
...
function redrawPageCanvas(i, cb) {
  var canvasWrapperNode = pageViews[i].div.getElementsByClassName(
    "canvasWrapper"
  )[0];
...
function afterPrintComplete() {
  for (var i = 0; i < pageViews.length; i++) {
    pageViews[i].viewport = pageViews[i].viewport.clone({
      scale: printPreviousScaleList[i] * (4 / 3),
    });
  ...
  if (pageCount > 100) {
    ...
    for (var i = 0; i < pageViews.length; i++) {
      (function (i) {
        ...
        if (needsScaleChange) {
          pageViews[i].viewport = pageViews[i].viewport.clone({
            scale: minimumAcceptableScale * (4 / 3),
          });
        }
