  ...
  input.split(",").forEach(function (option) {
    if (option.startsWith("domain=")) {
      ...
      */
      ...
      if (
        option[0] === "~" &&
        elementTypes.indexOf(option.substring(1)) !== -1
      ) {
        ...
        output.elementTypes =
          (output.elementTypes || 0) | elementTypesSet[option];
        hasValidOptions = true;
      }
...
function matchOptions(filterOptions, input, contextParams, currentHost) {
  ...
  if (contextParams.domain !== undefined) {
    ...
    if (
      ...
      filterOptions.skipDomains.indexOf(contextParams.domain) !== -1
    ) {
      return false;
    }
    ...
    if (
      ...
      filterOptions.domains.indexOf(contextParams.domain) === -1
    ) {
      return false;
    }
...
function parse(input, parserData, callback, options = {}) {
  ...
  for (var i = 0; i < arrayFilterCategories.length; i++) {
    parserData[arrayFilterCategories[i]] =
      parserData[arrayFilterCategories[i]] || [];
    parserData.exceptionFilters[arrayFilterCategories[i]] =
      parserData.exceptionFilters[arrayFilterCategories[i]] || [];
  }

  for (var i = 0; i < objectFilterCategories.length; i++) {
    parserData[objectFilterCategories[i]] =
      parserData[objectFilterCategories[i]] || {};
  ...
  for (var i = 0; i < trieFilterCategories.length; i++) {
    parserData[trieFilterCategories[i]] =
      parserData[trieFilterCategories[i]] || new Trie();
      ...
      if (parseFilter(filter, parsedFilterData)) {
        ...
        if (parsedFilterData.leftAnchored) {
          if (parsedFilterData.rightAnchored) {
            ...
            object.leftAnchored.add(
              parsedFilterData.data,
              parsedFilterData.options,
              maybeMergeDuplicateOptions
            );
          }
        } else if (parsedFilterData.rightAnchored) {
          object.rightAnchored.addReverse(
            parsedFilterData.data,
            parsedFilterData.options
          );
          ...
          var wildcardToken = parsedFilterData.wildcardMatchParts[0]
            .split("^")[0]
            .substring(0, 10);
          if (wildcardToken.length < 4) {
            var wildcardToken2 = parsedFilterData.wildcardMatchParts[1]
              .split("^")[0]
              .substring(0, 10);
            if (
              wildcardToken2 &&
              wildcardToken2.length > wildcardToken.length
            ) {
          ...
          object.nonAnchoredString.add(
            parsedFilterData.data.split("^")[0].substring(0, 10),
            parsedFilterData
          );
        }
      }
  ...
  if (options.async === false) {
    ...
    function nextChunk() {
      ...
      if (t2 - t1 !== 0) {
        nextChunkSize = Math.round(
          nextChunkSize / ((t2 - t1) / targetMsPerChunk)
        );
      }
...
function matchesFilters(filters, input, contextParams) {
  ...
  if (leftAnchoredMatches.length !== 0) {
    ...
    for (i = 0; i < len; i++) {
      if (
        matchOptions(leftAnchoredMatches[i], input, contextParams, currentHost)
      ) {
        return true;
      }
    }
  ...
  var rightAnchoredMatches = filters.rightAnchored.getEndingSubstringsOfReversed(
    input
  );
  if (rightAnchoredMatches.length !== 0) {
    ...
    for (i = 0; i < len; i++) {
      if (
        matchOptions(rightAnchoredMatches[i], input, contextParams, currentHost)
      ) {
        return true;
      }
    }
  ...
  for (i = 0, len = filters.bothAnchored.length; i < len; i++) {
    if (
      ...
      matchOptions(
        filters.bothAnchored[i].options,
        input,
        contextParams,
        currentHost
      )
  ...
  if (hostFiltersToCheck) {
    ...
    for (i = 0, len = hostFiltersToCheck.length; i < len; i++) {
      ...
      if (
        ...
        indexOfFilter(input, filter.data) !== -1 &&
  ...
  var nonAnchoredStringMatches = filters.nonAnchoredString.getSubstringsOf(
    input
  );
  ...
  if (nonAnchoredStringMatches.length !== 0) {
    ...
    for (var i = 0; i < len; i++) {
      ...
      if (
        matches &&
        matchOptions(
          nonAnchoredStringMatches[i].options,
          input,
          contextParams,
          currentHost
        )
