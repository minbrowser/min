...
var urlParser = {
  ...
  removeProtocol: function (url) {
    ...
    var withoutProtocol = url
      .replace("http://", "")
      .replace("https://", "")
      .replace("file://", ""); // chrome:, about:, data: protocols intentionally not removed
  ...
  parse: function (url) {
      ...
      try {
        ...
        if (/^[a-zA-Z]+$/.test(path)) {
          ...
          return urlParser.getFileURL(
            __dirname + "/pages/" + path + "/index.html" + urlObj.search
          );
        }
      } catch (e) {}
    }
    ...
    return searchEngine
      .getCurrent()
      .searchURL.replace("%s", encodeURIComponent(url));
  },
  basicURL: function (url) {
    return urlParser
      .removeProtocol(url)
      .replace(urlParser.trailingSlashRegex, "");
  },
  prettyURL: function (url) {
    try {
      var urlOBJ = new URL(url);
      return (urlOBJ.hostname + urlOBJ.pathname)
        .replace(urlParser.startingWWWRegex, "$1")
        .replace(urlParser.trailingSlashRegex, "");
    } catch (e) {
