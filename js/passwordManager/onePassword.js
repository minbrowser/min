...
class OnePassword {
  ...
  getLocalPath() {
    return path.join(
      ...
      "tools",
      platformType === "windows" ? "op.exe" : "op"
    );
  }
  ...
  async loadSuggestions(command, domain) {
    try {
      ...
      for (var i = 0; i < credentials.length; i++) {
        ...
        var usernameFields = credential.details.fields.filter(
          (f) => f.designation === "username"
        );
        var passwordFields = credential.details.fields.filter(
          (f) => f.designation === "password"
        );
      ...
      const { error, data } = ex;
      console.error(
        "Error accessing 1Password CLI. STDOUT: " + data + ". STDERR: " + error,
        ex
      );
      return [];
    }
  ...
  async signInAndSave(credentials, path = this.path) {
    ...
    const process = new ProcessSpawner(path, [
      "signin",
      "--raw",
      "my.1password.com",
      credentials.email,
