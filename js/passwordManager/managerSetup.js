...
var primaryInstructions = document.getElementById(
  "manager-setup-instructions-primary"
);
var secondaryInstructions = document.getElementById(
  "manager-setup-instructions-secondary"
);
...
const setupDialog = {
  ...
  show: function (manager) {
    ...
    document.getElementById("manager-setup-heading").textContent = l(
      "passwordManagerSetupHeading"
    ).replace("%p", manager.name);
    document.getElementById("password-manager-setup-link").textContent = l(
      "passwordManagerSetupLink"
    ).replace("%p", manager.name);
    document.getElementById(
      "password-manager-setup-link-installer"
    ).textContent = l("passwordManagerSetupLinkInstaller").replace(
      "%p",
      manager.name
    );
    ...
    if (setupDialog.setupMode === "installer") {
      ...
      setupDialog.installerCompletionTimeout = setTimeout(
  ...
  initialize: function () {
    document
      .getElementById("manager-setup-disable")
    ...
    document
      .getElementById("manager-setup-close")
      .addEventListener("click", function () {
        settings.set("passwordManager", null);
        setupDialog.hide();
      });
    ...
    document
      .getElementById("password-manager-setup-link")
      .addEventListener("click", function () {
        browserUI.addTab(
          tabs.add({
    ...
    document
      .getElementById("password-manager-setup-link-installer")
      .addEventListener("click", function () {
        browserUI.addTab(
          tabs.add({
            url: setupDialog.manager.getDownloadLink(),
    ...
    dragBox.ondrop = (e) => {
      ...
      if (
        ...
        !e.dataTransfer.files[0].name.endsWith(".exe")
      ) {
