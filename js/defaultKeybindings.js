...
const defaultKeybindings = {
  initialize: function () {
    ...
    for (var i = 1; i < 10; i++) {
      (function (i) {
        keybindings.defineShortcut({ keys: "shift+option+mod+" + i }, function (
          e
        ) {
          const taskSwitchList = tasks.filter((t) => !tasks.isCollapsed(t.id));
    ...
    keybindings.defineShortcut("closeAllTabs", function (d) {
    ...
    keybindings.defineShortcut("reload", function () {
      ...
      if (time - lastReload < 500) {
        ...
        webviews.update(
          tabs.getSelected(),
          new URL(tabs.get(tabs.getSelected()).url).searchParams.get("url")
        );
      } else {
