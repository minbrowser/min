...
const readerDecision = {
  ...
  setURLStatus(url, isReaderable) {
    ...
    readerDecision.info.URLStatus[url] = {
      lastVisit: Date.now(),
      isReaderable,
    };
    saveData();
  ...
  getSameDomainStatuses: function (url) {
    ...
    for (var itemURL in readerDecision.info.URLStatus) {
      try {
        if (
          new URL(itemURL).hostname === new URL(url).hostname &&
          itemURL !== url
        ) {
...
function saveData() {
  localStorage.setItem(
    "readerData",
    JSON.stringify({ version: 1, data: readerDecision.info })
  );
}
...
function cleanupData() {
  ...
  for (var url in readerDecision.info.URLStatus) {
    if (
      Date.now() - readerDecision.info.URLStatus[url].lastVisit >
      6 * 7 * 24 * 60 * 60 * 1000
    ) {
