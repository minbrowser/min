  ...
  showMenu: function (data, extraData) {
    ...
    if (link) {
      ...
      if (!currentTab.private) {
        linkActions.push({
          ...
          click: function () {
            browserUI.addTab(tabs.add({ url: link }), {
              enterEditMode: false,
    ...
    if (mediaURL && data.mediaType === "image") {
      clipboardActions.push({
        label: l("copy"),
        click: function () {
          webviews.callAsync(tabs.getSelected(), "copyImageAt", [
            data.x,
            data.y,
          ]);
        },
    ...
    menuSections.push([
      {
        ...
        click: function () {
          webviews
            ...
            .inspectElement(data.x || 0, data.y || 0);
        },
      },
    ]);
    ...
    var contextMenuScripts = userscripts
      .getMatchingScripts(tabs.get(tabs.getSelected()).url)
