const UPDATE_URL =
  "https://minbrowser.github.io/min/updates/latestVersion.json";
  ...
  if (settings.get("updateNotificationsEnabled") !== false) {
    ...
    fetch(UPDATE_URL, {
      ...
      .then(function (response) {
        ...
        if (
          response.version &&
          compareVersions(window.globalArgs["app-version"], response.version) >
            0 &&
          (!response.availabilityPercent ||
            getUpdateRandomNum() < response.availabilityPercent)
        ) {
...
function showUpdateNotification(text, input, event) {
  ...
  if (update) {
    ...
    if (
      compareVersions(window.globalArgs["app-version"], update.version) <= 0
    ) {
      return;
    }
