...
var searchbar = {
  ...
  getValue: function () {
    ...
    return text.replace(
      text.substring(
        searchbar.associatedInput.selectionStart,
        searchbar.associatedInput.selectionEnd
      ),
      ""
    );
  },
  showResults: function (text, event) {
    ...
    if (event && event.keyCode !== 8) {
      var realText =
        text.substring(0, searchbar.associatedInput.selectionStart) +
        event.key +
        text.substring(searchbar.associatedInput.selectionEnd, text.length);
    } else {
      var realText = text;
    }
