...
function showRestoreTask() {
  ...
  if (recentTabs.length === 1) {
    var title =
      searchbarUtils.getRealTitle(recentTabs[0].title) || l("newTabLabel");
  } else if (recentTabs.length === 2) {
    var title = l("taskDescriptionTwo")
      .replace(
        "%t",
        searchbarUtils.getRealTitle(recentTabs[0].title) || l("newTabLabel")
      )
      .replace(
        "%t",
        searchbarUtils.getRealTitle(recentTabs[1].title) || l("newTabLabel")
      );
  } else {
    var title = l("taskDescriptionThree")
      .replace(
        "%t",
        searchbarUtils.getRealTitle(recentTabs[0].title) || l("newTabLabel")
      )
