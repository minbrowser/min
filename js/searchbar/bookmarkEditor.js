...
const bookmarkEditor = {
  ...
  render: async function (url, options = {}) {
    ...
    bookmarkEditor.currentInstance.bookmark = await db.places
      .where("url")
      .equals(url)
      .first();
    ...
    if (!options.simplified) {
      ...
      saveButton.addEventListener("click", function () {
        editor.remove();
        bookmarkEditor.currentInstance.onClose(
    ...
    bookmarkEditor.currentInstance.bookmark.tags.forEach(function (tag) {
      tagArea.appendChild(
        bookmarkEditor.getTagElement(tag, true, function () {
    ...
    places.getSuggestedTags(
      bookmarkEditor.currentInstance.bookmark.url,
      function (suggestions) {
        ...
        tags.suggested
          ...
          .forEach(function (tag, idx) {
            tagArea.appendChild(
              bookmarkEditor.getTagElement(tag, false, function () {
                places.toggleTag(
                  bookmarkEditor.currentInstance.bookmark.url,
                  tag
                );
        ...
        newTagInput.addEventListener("change", function () {
          ...
          if (!tags.selected.includes(val)) {
            ...
            tagArea.insertBefore(
              bookmarkEditor.getTagElement(val, true, function () {
                places.toggleTag(
                  bookmarkEditor.currentInstance.bookmark.url,
                  val
                );
  ...
  show: function (url, replaceItem, onClose, options) {
    if (bookmarkEditor.currentInstance) {
      if (
        bookmarkEditor.currentInstance.editor &&
        bookmarkEditor.currentInstance.editor.parentNode
      ) {
