...
function addUnlockButton(target) {
  if (
    getUsernameFields().includes(target) ||
    getPasswordFields().includes(target)
  ) {
...
function checkInitialFocus() {
  if (
    getUsernameFields().includes(document.activeElement) ||
    getPasswordFields().includes(document.activeElement)
  ) {
...
ipc.on("password-autofill-match", (event, data) => {
  ...
  if (data.credentials.length === 0) {
    ...
    const firstField = getUsernameFields().filter(
...
function handleFormSubmit() {
  ...
  if (
    usernameValues.some((v) => v.length > 0) ||
    passwordValues.some((v) => v.length > 0)
  ) {
    ipc.send("password-form-filled", [
