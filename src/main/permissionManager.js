...
import { sendIPCToWindow } from './windowUtils'
...
function sendPermissionsToRenderer () {
    ...
    sendIPCToWindow(g.mainWindow, 'updatePermissions', pendingPermissions.concat(grantedPermissions).map(p => {
        return {
...
ipcMain.on('permissionGranted', function (e, permissionId) {
    for (var i = 0; i < pendingPermissions.length; i++) {
