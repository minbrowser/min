...
function downloadHandler(event, item, webContents) {
  ...
  if (
    ...
    // clicking the download button in the viewer opens a blob url, so we don't want to open those in the viewer (since that would make it impossible to download a PDF)
  ...
  ses.webRequest.onHeadersReceived(function (details, callback) {
    if (details.resourceType === "mainFrame" && details.responseHeaders) {
      ...
      if (
        ...
        details.url.indexOf("#pdfjs.action=download") === -1
      ) {
