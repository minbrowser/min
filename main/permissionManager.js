...
*/
function isPermissionGrantedForContents(
  requestContents,
  requestPermission,
  requestDetails
) {
  ...
  for (var i = 0; i < grantedPermissions.length; i++) {
    var grantedOrigin = new URL(grantedPermissions[i].details.requestingUrl)
      .hostname;

    if (
      requestContents === grantedPermissions[i].contents &&
      requestOrigin === grantedOrigin
    ) {
      if (
        requestPermission === "notifications" &&
        grantedPermissions[i].permission === "notifications"
      ) {
        return true;
      }
      ...
      if (
        ...
        if (
          ...
          requestDetails.mediaTypes.every((type) =>
            grantedPermissions[i].details.mediaTypes.includes(type)
          )
        ) {
          return true;
        }
...
function hasPendingRequestForContents(contents, permission, details) {
  ...
  for (var i = 0; i < pendingPermissions.length; i++) {
    var pendingOrigin = new URL(pendingPermissions[i].details.requestingUrl)
      .hostname;
  ...
  if (["media", "notifications"].includes(permission)) {
    ...
    */
    if (isPermissionGrantedForContents(webContents, permission, details)) {
      ...
      hasPendingRequestForContents(webContents, permission, details)
    ) {
      /*
    ...
    */
    webContents.on("did-start-navigation", function (
      e,
      url,
      isInPlace,
      isMainFrame,
