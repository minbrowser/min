...
function initFilterList() {
  ...
  fs.readFile(
    __dirname + "/ext/filterLists/easylist+easyprivacy-noelementhiding.txt",
    "utf8",
    function (err, data) {
      if (err) {
        return;
  ...
  fs.readFile(app.getPath("userData") + "/customFilters.txt", "utf8", function (
    err,
    data
  ) {
    if (!err && data) {
...
function handleRequest(details, callback) {
  ...
  if (
    ...
    !(domain && requestDomainIsException(domain))
  ) {
    if (
